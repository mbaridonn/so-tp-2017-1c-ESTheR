...
void inicializarLog(){
	cpu_log = log_create("/home/utnso/git/tp-2017-1c-C-digo-Facilito/CPU/Debug/CPU.log", "CódigoFacilito-CPU\n", true, LOG_LEVEL_TRACE);
}
...
void agregarSocket(int client_socket[], int *cliente) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		if (client_socket[i] == 0) {
			...
			log_info(cpu_log, "Agregando a conjunto de sockets como %d ", i);
...
void esperarConexion(int *servidor, struct sockaddr_in *direccionServidor) {
	...
	if (bind((*servidor), (void*) &(*direccionServidor),
		...
		log_error(cpu_log, "Falló el bind");
		//perror("Falló el bind. \n");
		exit(-1);
	}

	log_info(cpu_log, "Estoy escuchando");
...
void aceptarConexion(int *servidor, int *cliente) {
	...
	(*cliente) = accept((*servidor), (void*) &direccionCliente,
			&tamanioDireccion);
	log_info(cpu_log, "Recibí una conexión en %d!! ", (*cliente));
...
void recibirMensajeDe(int *cliente, char *buffer) {
	...
	if (bytesRecibidos <= 0) {
		log_error(cpu_log, "El chabón se desconectó o bla");
		//perror("El chabón se desconectó o bla. \n");
		exit(-1);
	}

	buffer[bytesRecibidos] = '\0';
	log_info(cpu_log, "Me llegaron %d bytes con %s ", bytesRecibidos, buffer);
...
void conectar(int *cliente, struct sockaddr_in *direccionServidor) {
	...
	if (connect((*cliente), (void*) &(*direccionServidor),sizeof((*direccionServidor))) != 0) {
		log_error(cpu_log, "No se pudo conectar");
