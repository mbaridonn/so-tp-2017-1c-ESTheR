...
int add_ret_var(t_stack_entry ** stack_entry, t_ret_var *ret_var) {
	...
	aux_buffer = realloc((*stack_entry)->ret_vars,
			((*stack_entry)->cant_ret_vars + 1) * sizeof(t_ret_var));
	if (aux_buffer == NULL) {
		return ERROR;
	}
	(*stack_entry)->ret_vars = aux_buffer;
	if (memcpy(((*stack_entry)->ret_vars + (*stack_entry)->cant_ret_vars),
			ret_var, sizeof(t_ret_var)) == NULL) {
		return ERROR;
	}
...
int add_var(t_stack_entry ** stack_entry, t_var *var) {
	void * aux_buffer = NULL;
	aux_buffer = realloc((*stack_entry)->vars,
			((*stack_entry)->cant_vars + 1) * sizeof(t_var));
	if (aux_buffer == NULL) {
		return ERROR;
	}
...
// int ret_pos, int cant_ret_vars, char *ret_vars})
	...
	void *stack_entry_item_buffer = NULL, *stack_list_buffer = *buffer, *next =
			NULL;
 ...
 * serialized_data_size : Tamanio total del conjunto de bytes.
 */
void deserialize_stack_entry(t_stack_entry **entry, void **serialized_data,
		int *serialized_data_size) {
	int cant_args = 0, cant_vars = 0, cant_ret_vars = 0;
	deserializar_data(&(*entry)->pos, sizeof(int), serialized_data,
			serialized_data_size);
	deserializar_data(&cant_args, sizeof(int), serialized_data,
			serialized_data_size);
//deserializar_data((*entry)->args, (size_t) (*entry)->cant_args, serialized_data, serialized_data_size);
	deserialize_args(entry, serialized_data, serialized_data_size, cant_args);
	deserializar_data(&cant_vars, sizeof(int), serialized_data,
			serialized_data_size);
//deserializar_data((*entry)->vars, (size_t) (*entry)->cant_vars, serialized_data, serialized_data_size);
	deserialize_vars(entry, serialized_data, serialized_data_size, cant_vars);
	deserializar_data(&cant_ret_vars, sizeof(int), serialized_data,
			serialized_data_size);
//deserializar_data((*entry)->ret_vars, (size_t) (*entry)->cant_vars, serialized_data, serialized_data_size);
	deserialize_ret_vars(entry, serialized_data, serialized_data_size,
			cant_ret_vars);
	deserializar_data(&(*entry)->ret_pos, sizeof(int), serialized_data,
			serialized_data_size);
}
void deserialize_args(t_stack_entry **entry, void **serialized_data,
		int *serialized_data_size, int cant_args) {
	int indice = 0;
	t_arg * aux_arg = NULL;
	if (cant_args > 0) {
		for (indice = 0; indice < cant_args; indice++) {
			aux_arg = calloc(1, sizeof(t_arg));
			deserializar_data(&aux_arg->page_number, sizeof(int),
					serialized_data, serialized_data_size);
			deserializar_data(&aux_arg->offset, sizeof(int), serialized_data,
					serialized_data_size);
			deserializar_data(&aux_arg->tamanio, sizeof(int), serialized_data,
					serialized_data_size);
			add_arg(entry, aux_arg);
		}
	}
}
void deserialize_vars(t_stack_entry **entry, void **serialized_data,
		int *serialized_data_size, int cant_vars) {
	int indice = 0;
	t_var * aux_var = NULL;
	if (cant_vars > 0) {
		for (indice = 0; indice < cant_vars; indice++) {
			aux_var = calloc(1, sizeof(t_var));
			deserializar_data(&aux_var->var_id, sizeof(char), serialized_data,
					serialized_data_size);
			deserializar_data(&aux_var->page_number, sizeof(int),
					serialized_data, serialized_data_size);
			deserializar_data(&aux_var->offset, sizeof(int), serialized_data,
					serialized_data_size);
			deserializar_data(&aux_var->tamanio, sizeof(int), serialized_data,
					serialized_data_size);
			add_var(entry, aux_var);
		}
	}
}
void deserialize_ret_vars(t_stack_entry **entry, void **serialized_data,
		int *serialized_data_size, int cant_ret_vars) {
	int indice = 0;
...
t_stack_entry* get_last_entry(t_stack * stack_index) {
	return (t_stack_entry*) list_get(stack_index->elements,
			stack_index->elements->elements_count - 1);
}
t_stack_entry* pop_stack(t_stack* stack_index) {
	return list_remove(stack_index->elements,
			stack_index->elements->elements_count - 1);
}
