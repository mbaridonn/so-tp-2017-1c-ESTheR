...
void hexDump16Bytes(void *addr, int len)//Sacada de otra página
{
    int i;
...
void atenderComandos() {
	...
	while (1) {
		...
		case 'd':
			...
			hexDump8Bytes(estructuraAdm, cantFrames * tamFrame);//NI IDEA SI FUNCIONA BIEN
		...
		case 's':
			...
			if (strcmp("memory", subcomando) == 0){
				...
				int PID = atoi(subcomando);//Si convierte algo que no es un int puede producir comportamiento inesperado
				if (0<PID && PID<cantFrames){//LÍMITE SUPERIOR COMPLETAMENTE ARBITRARIO
					printf("El proceso ocupa %d frames\n", cantPagsPorPID[PID]);
				}
				else{
					printf("'%s' no es un subcomando valido\n", subcomando);
				}
			}
			break;
		default:
...
int hash(int PID, int nroPag) {
	int indiceEnLaTabla = cantFramesEstructuraAdm;//comienza a buscar desde que terminan los frames de la estructura administrativa
